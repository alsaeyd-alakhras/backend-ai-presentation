...
- `assets/` — shared static resources (referenced from any `module-NN` lecture as `../../assets/...`):
  - `assets/*.jpeg`/`*.jpg` — shared images (instructor photo, etc).
  - `assets/css/deck.css` — the shared slide-deck stylesheet (all `:root` tokens, `.slide`/`.sb`/`.tb`, every reusable component class, animations, copy-button styles). Lectures link it with `<link rel="stylesheet" href="../../assets/css/deck.css" />` instead of an inline `<style>` block.
  - `assets/js/deck.js` — the shared slide-deck script (`go()` navigator, progress bar, keyboard nav, copy-button init). `N` (slide count) is derived from `document.querySelectorAll(".slide").length`, not hardcoded, so the same file works for any lecture. Lectures load it with `<script src="../../assets/js/deck.js"></script>` instead of an inline `<script>` block.
  - **Migration status:** all of module 1 (`lecture-01.html`–`lecture-05.html`) and module 2 (`lecture-06.html`, `lecture-07.html`) have been migrated to the shared CSS/JS and the light theme. A migrated lecture links `../../assets/css/deck.css` and, only if that module needs component classes or accent colors the shared stylesheet doesn't cover, an additional per-module companion stylesheet living next to the lecture files (e.g. `lectures/module-02/module-02.css`) — never duplicate what shared `deck.css` already defines, and never let a companion file's `:root` override the shared theme tokens. Remaining modules still carry their own inline `<style>`/`<script>` blocks — when editing one, prefer migrating it (shared CSS/JS, plus a companion stylesheet if genuinely needed) rather than re-duplicating logic, but don't silently migrate a file the user didn't ask about.
...
- JS must provide a `go()` navigator keyed off the real slide count, a `#progress`/`#progress-fill` bar updated on navigation, keyboard arrow support, `scaleStage()` + resize handler if `#scale-wrap` responsive scaling is used, and `initCopyButtons()` called once at the end. Lectures on the shared `assets/js/deck.js` get all of this for free (slide count is read from the DOM); lectures still on an inline `<script>` must keep `const N = <slide count>` in sync with the actual slide count by hand.
